...
DISABLE FLOW (normal — user has their phone):
  ...
  Step 1 — Biometric gate (Secure Enclave):
    User must pass Face ID / Touch ID
    Same hardware gate as any sensitive action

  Step 2 — Dual OTP confirmation:
    OTP sent to registered email AND registered phone simultaneously
    User must enter BOTH (dual confirmation for any security-lowering action)
    Why both: attacker with stolen phone still needs email access too
    Two separate channels → attacker must compromise both at once

  Step 3 — Time-limited disable:
    Disabled for maximum 24 hours — then auto re-enables
    Bank notified immediately: "User [device_id] disabled PhysicalOTP"
    Bank can: flag account, apply lower transaction limits, contact user

EMERGENCY DISABLE — PHONE LOST (ZKP recovery applies here):
  User has lost their phone and cannot tap the biometric gate.
  Standard disable flow is blocked — good, because attacker with stolen
  phone also cannot trigger this path silently.

  User calls bank / contacts support:
    Bank verifies identity: security questions + government ID + video selfie

  If identity verified — two paths depending on recovery method chosen at setup:

  PATH A — ZKP Recovery Code (most secure):
    User has their 24-word / alphanumeric recovery code on paper
    User enters code on NEW device in our app
    App generates ZKP proof locally (code never transmitted)
    Proof sent to our server → verified against stored commitment
    If valid: old device registration INVALIDATED immediately
              new device registered
              recovery code BURNED — permanently invalid
              user must write down a new recovery code
    Account access restored on new device
    Bank notified: "Device replaced via ZKP recovery — review recommended"

  PATH B — Bank issues one-time reset token:
    If user does not have recovery code available immediately:
    Bank admin issues a one-time reset token (30-minute window)
    This token ONLY unlocks the ZKP recovery entry screen
    It does not bypass it — user still must enter their recovery code
    Even the bank admin cannot skip the ZKP step
    This prevents bank staff from recovering accounts without user involvement

  DURING ANY DISABLE OR RECOVERY WINDOW:
    Transaction limits reduced automatically
    High-value actions require additional manual bank verification
    All activity monitored and flagged for fraud review
    The account is never fully unprotected — just at reduced security level
```

### Extension Removal Policy (SignedSend — Consumer vs Enterprise)

```
CONSUMER (individual, free tier):
  User can remove extension from Chrome at any time
  Before removal completes: extension sends removal signal to our server
  Our server pushes notification to registered phone:
    "⚠️ SignedSend removed from Chrome on your laptop
     If this was not you — tap EMERGENCY LOCK"
  User taps LOCK → all their email addresses flagged as unverified
  Recipients' extensions show: 🔴 EMAIL ACCOUNT SECURITY EVENT
  Even if user removed it themselves: subsequent emails arrive unsigned
  Recipients with extension see ⚠️ UNVERIFIED on all future emails
  This alone is the natural consequence — unsigned = suspect

ENTERPRISE (admin-deployed via MDM):
  Extension is policy-locked — cannot be removed by the end user at all
  Chrome Enterprise / MDM sets: ExtensionSettings policy
    "force_installed": true → extension cannot be uninstalled by user
    "blocked": true for removal → Chrome greyes out the remove button
  Attempting to remove: Chrome says "Managed by your organisation"
                        Remove button is disabled — cannot click it
  To remove: IT admin must update MDM policy → requires admin credentials
             Admin action logged → admin is notified of the policy change
             Change triggers review in our admin panel: policy event logged

  IF ENTERPRISE EXTENSION IS REMOVED (admin action or device wipe):
    Our server detects: device_id stopped sending heartbeat
    Alert sent to: IT admin + security team + our admin panel
    Policy violation logged: timestamp, device, admin who made change
    All email from that device flagged in our system until re-enrolled
    Recipients' extensions show: ⚠️ UNVERIFIED for emails from that address

  WHY THIS IS ENOUGH WITHOUT NEEDING A SPECIAL UNINSTALL PASSWORD:
    MDM policy lock is the uninstall prevention — it is OS-level enforcement
    No extension password can be stronger than MDM control
    If admin credentials are compromised: that is a separate security incident
      → Handled by IT, not by our extension
    The flag + alert system means even a successful removal is immediately visible
    You cannot hide the removal — it surfaces within seconds on our server
```
