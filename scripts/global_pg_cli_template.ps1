...
$DefaultRef = "main"
$GlobalCliVersion = "1.1.0"

function Get-GlobalRoot {
    return Split-Path -Parent $PSCommandPath
}

function Get-PayloadRoot {
    return Join-Path (Get-GlobalRoot) "payload\current"
}
...
function Show-Help {
    ...
    Write-Host ""
    Write-Host "Upgrade scaffold in current repo:"
    Write-Host "  pg install backend -UpgradeScaffold -DryRun"
    Write-Host "  pg install backend -UpgradeScaffold -Yes"
    Write-Host ""
    Write-Host "Install into specific path:"
    Write-Host "  pg install backend --target C:\path\to\repo"
    Write-Host "  pg install backend --target C:\path\to\repo -UpgradeScaffold -DryRun"
    Write-Host ""
...
function Parse-InstallArgs {
    ...
    while ($i -lt $Args.Count) {
        ...
        switch ($token) {
            ...
            "-UpgradeScaffold" { $upgradeScaffold = $true }
            "--upgrade-scaffold" { $upgradeScaffold = $true }
            "-DryRun" { $dryRun = $true }
            "--dry-run" { $dryRun = $true }
            "-Yes" { $yes = $true }
            "--yes" { $yes = $true }
            "-RunSelfCheck" { $runSelfCheck = $true }
            "--run-self-check" { $runSelfCheck = $true }
            "-BackupRoot" {
                $i++
                if ($i -ge $Args.Count) { throw "-BackupRoot requires a value." }
                $backupRoot = $Args[$i]
            }
            "--backup-root" {
                $i++
                if ($i -ge $Args.Count) { throw "--backup-root requires a value." }
                $backupRoot = $Args[$i]
            }
            "backend" { }
            "frontend" { }
            "mobile" { }
            default {
                throw "Unknown install argument: $token"
            }
        }
        $i++
    }
    ...
    return [PSCustomObject]@{
        ...
        Repo = $repo
        Ref = $ref
        UpgradeScaffold = $upgradeScaffold
        DryRun = $dryRun
        Yes = $yes
...
function Ensure-LocalPayload {
    $payloadRoot = Get-PayloadRoot
    $upgradeScript = Join-Path $payloadRoot "scripts\scaffold_upgrade.ps1"
    if (-not (Test-Path -LiteralPath $upgradeScript)) {
        throw "Global PG CLI payload is missing scaffold upgrade support. Reinstall or resync the latest PG CLI payload on this machine."
    }
    return $payloadRoot
}

function Invoke-LocalPayloadUpgrade {
    param([Parameter(Mandatory = $true)][pscustomobject]$Options)

    $payloadRoot = Ensure-LocalPayload
    $upgradeScript = Join-Path $payloadRoot "scripts\scaffold_upgrade.ps1"
    $resolvedTarget = Resolve-Path -LiteralPath $Options.TargetPath -ErrorAction SilentlyContinue
    if (-not $resolvedTarget) {
        throw "Target path not found: $($Options.TargetPath)"
    }
    ...
    if ($Options.Yes) { $args["Yes"] = $true }
    if ($Options.RunSelfCheck) { $args["RunSelfCheck"] = $true }
    if (-not [string]::IsNullOrWhiteSpace($Options.BackupRoot)) { $args["BackupRoot"] = $Options.BackupRoot }

    & powershell -ExecutionPolicy Bypass -File $upgradeScript @args
    exit $LASTEXITCODE
}
...
switch ($Command) {
    ...
    "version" {
        Write-Host "pg version: $GlobalCliVersion"
        Write-Host "default repo: $DefaultRepoSlug"
        Write-Host "payload root: $(Get-PayloadRoot)"
        exit 0
    }
    "install" {
        $opts = Parse-InstallArgs -Args $Rest -DefaultTarget $Target
        if ($opts.UpgradeScaffold) {
            Invoke-LocalPayloadUpgrade -Options $opts
        }
